...
export const systemStitchingGateManifestSchema = z
  .object({
    ...
    syntheticBasis: z
      .object({
        producerService: z.literal('orders-api-example'),
        consumerService: z.literal('orders-worker-example'),
        transport: z.literal('rmq'),
        queue: z.literal('orders_jobs'),
        pattern: z.literal('orders.rebuild-index'),
        ...
        producerSourceFiles: z.tuple([
          z.literal('src/modules/orders/orders.service.ts'),
          z.literal('src/modules/workflow/workflow.service.ts'),
          z.literal('src/modules/workflow/workflow.module.ts'),
        ]),
        consumerDecorator: z.literal('MessagePattern'),
        consumerSourceFiles: z.tuple([
          z.literal('src/main.ts'),
          z.literal('src/modules/orders/orders.controller.ts'),
        ]),
      })
      .strict(),
  })
