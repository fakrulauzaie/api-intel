...
import {
  ...
  locateCytoscapeBrowserSource,
  renderOfflineGraphReport,
  resetCytoscapeBrowserSourceCache,
...
describe('Phase 23 self-contained HTML rendering', () => {
  ...
  it('locates and caches the browser Cytoscape source accurately', async () => {
    resetCytoscapeBrowserSourceCache();
    const locatedPath = await locateCytoscapeBrowserSource();
    expect(locatedPath).toMatch(/cytoscape(?:\.min)?\.js$/u);

    const first = await loadCytoscapeBrowserSource();
    expect(first.length).toBeGreaterThan(100_000);
    expect(first).toContain('cytoscape');

    // Second call should return the cached instance
    const second = await loadCytoscapeBrowserSource();
    expect(second).toBe(first);
  });
});
