...
describe('offline system report application', () => {
  ...
  it('opens on a compact path-only service to broker to service layout', () => {
    const report = {
      graph: {
        nodes: [
          node('ticket', 'service', 'orders-api', null),
          node('producer', 'producer', 'producer', 'ticket', ['correlation']),
          node('unmatched', 'producer', 'unmatched inventory', 'ticket'),
          node('realm', 'broker_realm', 'orders-rmq', null),
          node('destination', 'broker_destination', 'orders_jobs', 'realm', ['correlation']),
          node('worker', 'service', 'orders-worker', null),
          node('consumer', 'consumer', 'consumer', 'worker', ['correlation']),
          node('effect', 'table_effect', 'WRITE audit_log', 'worker', ['correlation']),
        ],
        edges: [
          {
            id: 'route',
      ...
      correlations: [
        {
          ...
          contractLabel: 'orders.rebuild-index',
          reason: null,
