...
function realm(input?: {
  ...
  const identity = {
    ...
    transport: 'rmq',
    destination: { kind: 'queue', value: 'orders_jobs' },
    prefix: null,
    namespace: null,
  } as const;
...
describe('Phase 45 system identity and correlation contract', () => {
  it('namespaces identical source IDs without merging source analysis documents', () => {
    const left = service('orders-api');
    const right = service('orders-worker');
  ...
  it('represents the synthetic orders-API-to-worker RMQ pair only as a declared-realm candidate', () => {
    const ticket = service('orders-api');
    const queue = service('orders-worker');
  ...
  it('keeps request fan-out ambiguous and rejects merged source-analysis fields', () => {
    const producerService = service('orders-api');
    const consumerAService = service('orders-worker-a');
    const consumerBService = service('orders-worker-b');
    const sharedRealm = realm();
    const producer = endpoint({
