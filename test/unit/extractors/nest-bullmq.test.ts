  ...
  it('resolves aliased as-const queue names and enum job names inside the repository', async () => {
    ...
    try {
      ...
      await project.write(
        'share/constants.ts',
        [
          "export enum VerificationJob { ORDERS_INDEX_API_SYNC = 'orders-index-api-sync' }",
      ...
      await project.write(
        ...
        [
          ...
          '    await this.verificationQueue.add(VerificationJob.ORDERS_INDEX_API_SYNC, {});',
          '  }',
          '}',
        ].join('\n'),
      );
      ...
      expect(result.analysis.interactions.filter(({ kind }) => kind === 'job_queue')).toEqual([
        expect.objectContaining({
          target: {
            ...
            job: { resolution: 'exact', value: 'orders-index-api-sync' },
          },
        }),
      ]);
