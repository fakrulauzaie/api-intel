...
describe('bounded critical-section wrapper propagation', () => {
  it('proves the representative two-level shape while rejecting every close W0 forwarding negative', async () => {
    ...
    try {
      ...
      expect(projectedFlows(verified.extraction)).toEqual([
        ...
        {
          method: 'RedisLockService.executeWithWorkflowLock',
          parameter: 'task',
          parameterIndex: 1,
          hops: 1,
          flow: [
      ...
      expect(
        ...
        },
        {
          ...
          target: 'RedisLockService.executeWithWorkflowLock',
          callbackArgumentIndex: 1,
          terminals: ['this.redlock.using'],
        },
      ]);
