...
(() => {
  'use strict';
  const embeddedData = JSON.parse(document.getElementById('api-intel-system-data').textContent);
  const documentData = embeddedData.report || embeddedData;
  const impactData = embeddedData.impact || null;
  const nodeRecords = new Map(documentData.graph.nodes.map((node) => [node.id, { ...node, impact: null, impactPathIds: [] }]));
  const edgeRecords = new Map(documentData.graph.edges.map((edge) => [edge.id, { ...edge, impact: null, impactPathIds: [] }]));
  if (impactData) {
    for (const overlay of impactData.overlay.nodes) {
      const prior = nodeRecords.get(overlay.nodeId);
      nodeRecords.set(overlay.nodeId, {
        ...(prior || {
          id: overlay.nodeId,
          label: overlay.label,
          kind: overlay.kind,
          parentId: overlay.parentNodeId,
          serviceId: null,
        ...
        impact: overlay.classification,
        impactPathIds: overlay.pathIds,
      });
    }
    for (const overlay of impactData.overlay.edges) {
      const prior = edgeRecords.get(overlay.edgeId);
      edgeRecords.set(overlay.edgeId, {
        ...(prior || {
          id: overlay.edgeId,
          source: overlay.source,
          target: overlay.target,
          label: overlay.kind.replaceAll('_', ' '),
          kind: overlay.kind,
          certainty: 'conditional_candidate',
          correlationId: null,
          diagnosticIds: [],
  ...
  const graphNodes = [...nodeRecords.values()];
  const graphEdges = [...edgeRecords.values()];
  ...
  const impactViewButton = document.getElementById('view-impact');
  ...
  const elements = [
    ...graphNodes.map((node) => ({ data: {
      ...
      impact: node.impact,
      impactPathIds: node.impactPathIds,
    }})),
    ...graphEdges.map((edge) => ({ data: {
      id: edge.id,
      source: edge.source,
      target: edge.target,
      label: edge.label,
      ...
      impact: edge.impact,
      impactPathIds: edge.impactPathIds,
    }})),
  ];
  const cy = cytoscape({
    ...
    style: [
      ...
      { selector: 'node[impact = "distributed_conditional"]', style: { 'border-color': '#ffde59', 'border-width': 5, 'overlay-color': '#ffde59', 'overlay-opacity': .09, 'overlay-padding': 9 } },
      { selector: 'edge[impact = "distributed_conditional"]', style: { 'line-color': '#ffde59', 'target-arrow-color': '#ffde59', 'width': 5, 'opacity': 1 } },
  ...
  const setViewButtons = (mode) => {
    ...
    if (impactViewButton) impactViewButton.setAttribute('aria-pressed', String(mode === 'impact'));
  };
  const showView = (requestedNodeIds, requestedEdgeIds, label, mode) => {
    const visibleNodeIds = withAncestors(requestedNodeIds);
    const visibleEdgeIds = new Set(graphEdges.filter((edge) =>
  ...
  const allEdgeIds = new Set(graphEdges.map((edge) => edge.id));
  const pathNodeIds = new Set(graphEdges.flatMap((edge) => [edge.source, edge.target]));
  ...
  const showImpact = () => {
    const impactEdges = graphEdges.filter((edge) => edge.impact === 'distributed_conditional');
    const impactEdgeIds = new Set(impactEdges.map((edge) => edge.id));
    const impactNodeIds = new Set([
      ...graphNodes.filter((node) => node.impact === 'distributed_conditional').map((node) => node.id),
      ...impactEdges.flatMap((edge) => [edge.source, edge.target]),
    ]);
    showView(impactNodeIds, impactEdgeIds, 'Potential distributed impact', 'impact');
  };
  ...
  const renderList = () => {
    ...
    for (const record of matches) {
      ...
      button.addEventListener('click', () => {
        ...
        const recordEdges = graphEdges.filter((edge) => edge.correlationId === record.id);
        ...
        const recordNodeIds = new Set([
          ...
          ...graphNodes.filter((node) => node.correlationIds.includes(record.id)).map((node) => node.id),
  ...
  cy.on('tap', 'node', (event) => {
    ...
    inspect(source.label, [
      ...
      ['Potential impact', source.impact || '—'],
  ...
  if (impactViewButton) impactViewButton.addEventListener('click', () => {
    for (const candidate of correlationList.querySelectorAll('button')) candidate.classList.remove('active');
    showImpact();
  });
