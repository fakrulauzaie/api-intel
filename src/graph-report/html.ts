...
let cachedCytoscapeSource: string | undefined;

export async function locateCytoscapeBrowserSource(): Promise<string> {
  const currentDirectory = dirname(fileURLToPath(import.meta.url));
  const assetFilename = 'cytoscape.min.js';
  // Construct this at runtime so ncc does not copy dependency-tree-relative duplicates;
  // the provider build owns one explicit, fingerprinted asset beside its chunks.
  const bundledAssetPath = `${currentDirectory}${sep}${assetFilename}`;
  try {
    const stats = await stat(bundledAssetPath);
    if (stats.isFile() && stats.size > 0) {
      return bundledAssetPath;
    }
  } catch {
    // Normal package/CLI builds resolve the exact dependency below.
  }

  try {
    const specifier = ['cytoscape', 'dist', 'cytoscape.min.js'].join('/');
    return require.resolve(specifier);
  } catch (error) {
    throw new GraphReportRenderError(
      `Unable to locate cytoscape.min.js for offline graph report rendering: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}

export function resetCytoscapeBrowserSourceCache(): void {
  cachedCytoscapeSource = undefined;
}

export async function loadCytoscapeBrowserSource(): Promise<string> {
  if (cachedCytoscapeSource !== undefined) {
    return cachedCytoscapeSource;
  }
  const path = await locateCytoscapeBrowserSource();
  const contents = await readFile(path, 'utf8');
  cachedCytoscapeSource = contents;
  return contents;
}
