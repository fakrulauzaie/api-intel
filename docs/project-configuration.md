...
When the package is installed as a local project dependency, editors may resolve the
configuration schema from the reviewed data export:
...
Repository contributors may continue to use `./schemas/api-intel.config.schema.json`.
The installed-package path is present in the O2.2 tarball and was resolved from both
isolated npm and pnpm consumers by the O2.3 clean-room gate.

`api-intel init .` previews a minimal current configuration and `api-intel init .
--write` creates it only when TypeScript proves NestJS declarations and no configuration
already exists. The generated file intentionally omits `$schema`: a global, npx, or
workspace invocation cannot prove which package-relative editor path will remain valid.
Initialization never overwrites and never infers the optional settings documented below.
