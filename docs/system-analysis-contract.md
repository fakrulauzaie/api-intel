...
## Real orders-API-to-worker basis

The frozen RMQ slice is derived from the supplied repositories:

- `orders-api-example` injects `MESSAGE_QUEUE_SERVICE`, registers RMQ queue
  `orders_jobs`, and calls both `ClientProxy.send()` and `ClientProxy.emit()` with
  pattern `orders.rebuild-index` from its orders/workflow services; and
- `orders-worker-example` connects an RMQ microservice to the same queue and
  declares `@MessagePattern('orders.rebuild-index')` in its orders controller.
...
`test/fixtures/system-stitching/orders.topology.json`. Its
...
pnpm run cli -- stitch `
  ...
  orders-worker=.\reports\orders\analysis.json `
