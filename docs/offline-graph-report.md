...
During generation, a normal package/CLI installation resolves that pinned dependency
from its package tree. The standalone GitHub and GitLab ncc distributions instead ship
an exact copy named `cytoscape.min.js` beside their emitted chunks; the renderer checks
that trusted module directory first and falls back to package resolution. It never
searches the analyzed repository or the process entrypoint for a substitute asset.
The resulting HTML is identical in its offline boundary either way.
...
The deterministic/offline/CSP/accessibility contracts and one browser-unspecified
manual pass do not establish a named browser support matrix. The current alpha claim
and the exact future manual checklist are recorded in
