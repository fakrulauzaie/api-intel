...
The sanitized synthetic RMQ basis is `orders-api-example` to `orders-worker-example` through
queue `orders_jobs` and pattern `orders.rebuild-index`. It is a declared-realm
