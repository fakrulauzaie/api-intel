...
Phase W6 validates analysis v8 against the supplied `orders-api-example` without
...
```powershell
pnpm run cli -- scan "<orders-api-example>" --no-config --tsconfig tsconfig.json `
  --output ".tmp\phase-w6\second" --max-call-depth 3 --controller WorkflowController `
...
`RedisLockService.executeWithWorkflowLock`. Exact symbol and parameter proof follows this
chain:

```text
WorkflowService.resolveTicket inline callback
  -> RedisLockService.executeWithWorkflowLock task parameter
...
including `buildResolveWorkflowAuditLog`, `getResolvableWorkflowOrThrow`, validation helpers,
`saveWorkflowResolutionInfo`, `WorkflowData.update`, resolution verification,
...
as `critical_section_conditional`: reads of `orders`, `order_activity`, `workflow`,
`workflow_network_element`, and `workflow_resolution_info`, plus writes of `workflow`, `workflow_note`,
and `workflow_resolution_info`.

The pre-existing `PUT /force-resolve` path remains expanded. Its ordinary work still
reports seven synchronous table effects, while the nested `forceResolveWorkflow` wrapper
...
- `CRITICAL_SECTION_CALLBACK_FLOW_UNPROVEN` on
  `WorkflowVerificationService.executeWorkflowVerificationMachine`, where a local
  `executeLogic` variable is passed to `executeWithWorkflowLock`. Analysis v8 intentionally
