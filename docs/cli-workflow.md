...
The separate `api-intel-mcp` binary is not a fourteenth source/report command. It starts
an artifact-only stdio process from explicitly named completed JSON files and never
scans. It exposes the bounded P0 query operations and exact selected-record resources
without adding CLI source-analysis semantics; see
[Local Artifact MCP Server](mcp-server.md) for its startup and adapter contract.

The CLI has thirteen complete commands:

```text
api-intel doctor [repository] [--config <path> | --no-config]
  [--tsconfig <path>] [--output <directory>]
  [--format text|json] [--probe-output]
...
`doctor` is the read-only-by-default preflight for repository, TypeScript program,
configuration, framework declaration, runtime asset, version-evidence, and capability
checks. It defaults to the current directory and never installs dependencies or
executes target code. Its normal output-path check is metadata-only; the separately
explicit `--probe-output` performs one bounded create/delete test. See
[Doctor and Capability Preflight](doctor.md).

`init` reuses the configuration-disabled doctor preflight and requires
TypeScript-resolved NestJS declarations. It previews the exact minimal version-4 JSON
by default; only `--write` creates `<repository>/api-intel.config.json`. Creation is
exclusive and there is no overwrite mode. Optional analysis and report choices remain
uninferred. See [First Evidence-Backed Trace](first-use-workflow.md).
