...
```powershell
pnpm run cli -- stitch `
  orders-api=.\reports\orders-api\.api-intel `
  orders-worker=.\reports\orders\.api-intel `
  --topology test\fixtures\system-stitching\orders.topology.json `
  --output .\reports\orders-system `
  --with-graph --open
```
...
P4.2 can render the same validated report with a side-specific impact overlay through
`renderOfflineSystemImpactReport({ report, impact, side })`. That artifact opens in
**Potential impact** view, includes any bounded overlay-only cascade edges omitted from
the base display, and highlights only paths seeded by a differential local change. It
does not change `SystemReportDocument` schema `1.0.0` or make ordinary `stitch
--with-graph` differential.
...
used by the orders worker service:

```typescript
return this.handleMessage(
  ctx,
  'label',
  this.ordersService.rebuildOrderIndex.bind(this.ordersService),
  dto,
);
```
