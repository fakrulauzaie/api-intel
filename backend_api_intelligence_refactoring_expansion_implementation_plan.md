...
Primary validation target: `orders-api-example` plus frozen synthetic fixtures
...
- The orders-api shape `VerificationJob.ORDERS_INDEX_API_SYNC` resolves to its exact
...
- A built-CLI scan of `orders-api-example` resolved all five observed
...
- Frozen 14-case BullMQ/Nest-microservice corpus covers co-located, producer-only,
  ...
  orders-api/orders-worker RMQ relationship is preserved through queue
  `orders_jobs` and pattern `orders.rebuild-index`.
...
- Froze the orders API/worker topology declaration for RMQ queue `orders_jobs` and pattern
  `orders.rebuild-index`. The existing ticket artifact passed a built-CLI smoke test (15
  endpoints projected with no absolute path retained). The supplied orders source confirms
...
- Added checker-proven same-class call extraction plus a bounded bound-callback
  ...
  directly and refuses retained/forwarded callbacks. This resolves the supplied orders
  `handleMessage(..., this.ordersService.rebuildOrderIndex.bind(...))` pattern without
  adding generic data-flow guesses.
- Fresh built-CLI scans of the installed orders API/worker repositories and a stitch through
  output paths containing spaces produced three declared-realm conditional paths. One
  begins at `POST /mobile/create`; all reach the orders candidate handler and its
  conditional `WRITE audit_log` effect. Existing source gaps remain explicit.
