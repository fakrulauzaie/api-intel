...
Primary validation target: `orders-api-example`
...
shape is the `RedisLockService.executeWithWorkflowLock()` chain used by
`WorkflowService.resolveTicket()`, but every published fact must follow from a bounded,
...
1. Freeze non-executable `.ts.txt` fixtures for a direct wrapper and the representative two-level
   `executeWithWorkflowLock` forwarding shape.
...
- Froze the direct wrapper and representative two-level
  `executeWithWorkflowLock → executeWithLock → Redlock.using` source shapes as
...
- Recorded the six required evidence roles, three-hop bound, analysis-v8 schema
  decision, v7 freeze, derived-technology rule, and synthetic orders-api basis.
...
- `executeWithWorkflowLock(..., task) → executeWithLock(..., task)` inherits the proven
...
- Verified the frozen two-level `executeWithWorkflowLock → executeWithLock` chain while the
...
1. Build the CLI and scan `orders-api-example` without executing application code.
2. Confirm `WorkflowService.resolveTicket()` traverses through
   `RedisLockService.executeWithWorkflowLock()` and exposes the expected inner calls and DB
   effects.
...
- Built the production CLI and scanned `orders-api-example` with `--no-config`,
...
- `PUT /resolve` now retains `RedisLockService.executeWithWorkflowLock` and traverses its
...
- `PUT /force-resolve` retained seven synchronous table effects and its nested
  `forceResolveWorkflow` section contributes two conditional table effects, proving that
